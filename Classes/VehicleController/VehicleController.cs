...
public class VehicleController : MonoBehaviour
{
    ...
    [SerializeField] private GameObject[] m_CaliperMeshes = new GameObject[4];
    ...
    public void Move(float steering, float accel, float footbrake, float handbrake)
    {
        for (int i = 0; i < 4; i++)
        {
            ...
            if (m_CaliperMeshes[i] != null)
            {
                m_CaliperMeshes[i].transform.position = position;
                m_CaliperMeshes[i].transform.localEulerAngles = new Vector3(0, 0, m_WheelColliders[i].steerAngle);
            }
        }
